...
const ADMIN_MAX_ATTEMPTS    = 5;     // จำนวนครั้งที่ใส่รหัสผิดได้ก่อนถูกล็อก
const ADMIN_LOCKOUT_SECONDS = 300;   // ระยะเวลาล็อกเมื่อใส่รหัสผิดเกินกำหนด (วินาที)
...
// escapeHtml_ — กันการฝัง HTML/แท็กจากข้อมูลที่ผู้ใช้กรอกเอง ก่อนนำไปแทรกใน
// อีเมล HTML หรือข้อความ Telegram (parse_mode: "HTML" จะ error ถ้าเจอแท็กที่ไม่ปิด)
...
// checkAdminPassword_ — ตรวจรหัสผ่าน admin พร้อมล็อกชั่วคราวเมื่อใส่ผิดซ้ำๆ
// (กันการไล่เดารหัสผ่านผ่าน google.script.run)
function checkAdminPassword_(password) {
  var cache   = CacheService.getScriptCache();
  var userKey = "adminAuth_" + Session.getTemporaryActiveUserKey();
  var lockKey = userKey + "_lock";

  if (cache.get(lockKey)) {
    return { ok: false, message: "ใส่รหัสผ่านผิดเกินกำหนด กรุณาลองใหม่อีกครั้งภายหลัง" };
  }

  if (password !== ADMIN_PASSWORD) {
    var fails = parseInt(cache.get(userKey) || "0", 10) + 1;
    if (fails >= ADMIN_MAX_ATTEMPTS) {
      cache.put(lockKey, "1", ADMIN_LOCKOUT_SECONDS);
      cache.remove(userKey);
    } else {
      cache.put(userKey, fails.toString(), ADMIN_LOCKOUT_SECONDS);
    }
    ...
    reasonBlock =
      '<tr><td style="padding:14px 28px 0;">' +
        '<div style="background:#fff7f7;border:1px solid #fecaca;border-radius:10px;padding:14px 16px;">' +
          ...
          '<div style="font-size:14px;color:#7f1d1d;line-height:1.6;">' + escapeHtml_(opts.reason || "ไม่ระบุ") + '</div>' +
        '</div>' +
      '</td></tr>';
  }
...
'<div style="background:#f1f5f9;padding:32px 16px;font-family:Tahoma,Arial,sans-serif;">' +
  '<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;margin:0 auto;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 18px rgba(15,23,42,.08);">' +
    '<tr><td style="background:linear-gradient(135deg,#4f46e5,#7c3aed);background-color:#4f46e5;padding:28px 28px 22px;">' +
      '<div style="display:inline-block;background:rgba(255,255,255,.18);color:#e0e7ff;font-size:11px;font-weight:600;letter-spacing:.5px;padding:4px 12px;border-radius:100px;margin-bottom:10px;">SPK OFFICE</div><br>' +
    ...
    '<tr><td style="padding:26px 28px 6px;">' +
      ...
      '<p style="font-size:14px;color:#334155;line-height:1.8;margin:10px 0 4px;">เรียน คุณ' + escapeHtml_(opts.name) + '</p>' +
    ...
    '<tr><td style="padding:16px 28px 4px;">' +
      '<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;">' +
        '<tr><td style="padding:16px 18px;font-size:13.5px;color:#334155;line-height:2;">' +
          '<b style="color:#1e293b;">ห้องประชุม</b>&nbsp;&nbsp;' + escapeHtml_(opts.room) + '<br>' +
          '<b style="color:#1e293b;">วันที่</b>&nbsp;&nbsp;' + formatDateThaiLong_(opts.date) + '<br>' +
          '<b style="color:#1e293b;">เวลา</b>&nbsp;&nbsp;' + escapeHtml_(opts.start) + ' – ' + escapeHtml_(opts.end) + ' น.<br>' +
          '<b style="color:#1e293b;">โครงการ/กิจกรรม</b>&nbsp;&nbsp;' + escapeHtml_(opts.project) +
          (opts.qty ? '<br><b style="color:#1e293b;">จำนวนผู้เข้าร่วม</b>&nbsp;&nbsp;' + escapeHtml_(opts.qty) + ' คน' : '') +
        '</td></tr>' +
      '</table>' +
    '</td></tr>' +
    reasonBlock +
...
function approveReservation(password, rowId) {
  ...
  var currentStatus = sheet.getRange(realRow, 13).getValue();
  if (currentStatus === "อนุมัติ" || currentStatus === "ปฏิเสธ") {
    return { success: false, message: "รายการนี้ถูกพิจารณาไปแล้ว (" + currentStatus + ") จึงไม่สามารถดำเนินการซ้ำได้" };
  }
  ...
  var maidMsg = "🧺 <b>การจองห้องประชุมได้รับการอนุมัติแล้ว — กรุณาเตรียมการ</b>\n\n"
    + "📍 ห้อง    : " + escapeHtml_(room) + "\n"
    + "📅 วันที่   : " + formatDateThaiLong_(date) + "\n"
    + "⏰ เวลา    : " + escapeHtml_(start) + " – " + escapeHtml_(end) + " น.\n"
    + "📌 เรื่อง   : " + escapeHtml_(project) + "\n"
    + "👤 ผู้จอง   : " + escapeHtml_(name) + " (" + escapeHtml_(position) + ")\n"
    + "☎️ โทร     : " + escapeHtml_(phone) + "\n"
    + "👥 จำนวน   : " + escapeHtml_(qty) + " คน\n"
    + "🍱 รายการ  : " + escapeHtml_(equipment || "-") + "\n\n"
...
function testTelegramMaebaan() {
  var ok = sendTelegramNotification_("🔧 ทดสอบการแจ้งเตือนไปยังแม่บ้าน (ลบข้อความนี้ทิ้งได้)", TELEGRAM_MAEBAAN_CHAT_ID);
  Logger.log("ผลการทดสอบส่งไปยังแม่บ้าน: " + (ok ? "✅ สำเร็จ" : "❌ ไม่สำเร็จ — ดู log ด้านบนเพื่อดูสาเหตุจาก Telegram"));
}
...
function saveReservation(formData) {
  try {
    ...
    var msg = "🔔 <b>มีคำขอจองห้องประชุมใหม่!</b>\n\n"
      + "👤 ผู้จอง : " + escapeHtml_(formData.requester_name) + "\n"
      + "📍 ห้อง   : " + escapeHtml_(formData.room) + "\n"
      + "📅 วันที่  : " + escapeHtml_(formData.date) + "\n"
      + "⏰ เวลา   : " + escapeHtml_(formData.start_time) + " – " + escapeHtml_(formData.end_time) + " น.\n"
      + "📌 เรื่อง  : " + escapeHtml_(formData.project_name) + "\n"
      + "👥 จำนวน  : " + escapeHtml_(formData.participants) + " คน\n"
      + "🍱 รายการ : " + escapeHtml_(eq.join(", ") || "-") + "\n"
      + "📧 อีเมล  : " + escapeHtml_(formData.email || "-") + "\n\n"
